...
if [[ "$LAYER" == "unit" || "$LAYER" == "all" ]]; then
  ...
  # Спеки рядом с исходниками (src/**/*.spec.ts) — ОТДЕЛЬНЫМ прогоном.
  #
  # До 27.08.2026 гейт их не гонял вовсе: только tests/unit, 21 сьют и 97
  # тестов. Остальные 1030 тестов, включая весь голосовой звонок, держались
  # на ручных прогонах — то есть на том, вспомнит ли живой человек их
  # запустить. За один день это дважды кончилось выкатом с поломкой в
  # соседнем месте.
  #
  # Прогон отдельный, а не одной командой с unit/, потому что у них разные
  # конфиги jest: unit-тесты живут в tests/ со своим, спеки — в корне репо.
  #
  # Путь ограничен src/ намеренно: без него jest подхватывает и .worktrees/,
  # где лежат чужие незаконченные ветки, и гейт краснеет от чужой работы.
  if [[ "$FAILED" -eq 0 ]] && ! (cd "$REPO_ROOT" && npx jest src/ --silent); then
    echo "  ✗ src specs failed"
    FAILED=1
  fi
fi

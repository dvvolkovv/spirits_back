...
[[ -f "$SRC/package.json" ]] || die "не нахожу product-runner рядом со скриптом: $SRC"
...
echo "URL=$u"
echo "TOKLEN=${#t}"
# [:graph:] — печатные без пробела, ровно TOKEN_SHAPE из host.guard.ts.
# Заодно ловит файл в CRLF: \r остаётся в значении и графическим символом не
# является, а вечный 401 от хвостового \r ничем не отличим от неверного токена.
...
bold "[6/7] запускаю"
# Перезапуск посреди задания убивает вместе с агентом и его docker run:
# подчистка не отрабатывает, на хосте остаётся полусозданный продукт, а
# владелец через десять минут читает про истёкший срок. Десять минут — это и
# есть серверный срок заведения, поэтому окно такое.
BUSY=$(remote <<'EOS'
set -uo pipefail
...
if [[ -n "$BUSY" && "$BUSY" != "0" ]]; then
  [[ "${FORCE:-}" == "1" ]] \
    || die "агент за последние 10 минут брал задание ($BUSY шт.) и, возможно, разворачивает продукт прямо сейчас.
      Перезапуск оставит на хосте полусозданный продукт. Дождаться (journalctl -u $UNIT -f) или FORCE=1"
  warn "перезапускаю поверх возможного задания по FORCE=1"
fi
START_TS=$(remote <<'EOS'
set -euo pipefail
date +%s

...
# каждую машину поимённо. Три части системы стоят на каждой из них и до
...
#   3. ОБРАЗ СРЕДЫ ПРОДУКТА (linkeon-product) — в нём запечён РАННЕР, который
#      правит код клиентского продукта. Живёт не на машине, а ВНУТРИ контейнера
#      каждого продукта. Катается scripts/product-image-roll.sh (добавлен
#      23.09.2026). До него образ не катился ничем: собран руками 09.09.2026,
#      установщик агента лишь проверял, что образ существует. Фаза при этом
#      рапортовала «машина синхронна с прод-коммитом», имея в виду только
#      агента, — а раннер у клиентов оставался сентябрьским, и ни одна его
#      починка до продуктов не доезжала (живой случай: падение на `git push`,
#      правка ассистентом не работала ни на одном продукте).
#
...
ensure_nginx_domains() {
  ...
  out=$(ssh_remote "
    ...
    printf '%s' '$hook_b64' | base64 -d | $PH_SUDO tee \"\$HT\" >/dev/null
...
# ── ЧЕТВЁРТАЯ ЧАСТЬ: ОБРАЗ СРЕДЫ ПРОДУКТА И КОНТЕЙНЕРЫ НА НЁМ ─────────────────
#
# Три части выше выкатывают то, что живёт НА машине: агента хоста, product-vhost
# и их конфиги. Но раннер, который правит код КЛИЕНТСКОГО продукта, живёт не на
# машине, а ВНУТРИ контейнера — он запечён в образ linkeon-product:base. Образ
# был собран руками 09.09.2026 и не катился ничем: установщик агента лишь
# проверял, что образ существует (product-host-agent-install.sh:156).
#
# Из-за этого фаза была ЧЕСТНОЙ РОВНО НАПОЛОВИНУ. Она говорила «машина синхронна
# с прод-коммитом», имея в виду агента, — а раннер внутри контейнеров оставался
# на сентябрьском коде. Любая его починка не доезжала до продуктов вовсе: правка
# в репозитории, зелёные тесты, выкаченный бэкенд — и ни одного изменения у
# клиента. Так уже вышло с падением раннера на `git push`: правка ассистентом не
# работала ни на одном продукте, а починка ничего не меняла.
#
# Выкат образа вынесен ОТДЕЛЬНЫМ скриптом (scripts/product-image-roll.sh) по той
# же причине, по которой установка агента — это product-host-agent-install.sh:
# у него свой сухой прогон, своя репетиция на одноразовом контейнере и свои
# проверки, и повторять их здесь значило бы завести второй способ делать то же
# самое. Здесь — только вызов и разбор кода возврата.
#
# КОММИТ ПЕРЕДАЁТСЯ ЯВНО (SOURCE_SHA=$PH_PROD_SHA). Скрипт умеет спросить HEAD
# прода и сам, но тогда он спросил бы его ВТОРОЙ РАЗ и мог бы получить другой
# ответ — чужой деплой в соседней сессии между нашими двумя вопросами. Обе
# половины фазы обязаны ехать из ОДНОГО коммита; ровно это и есть инвариант, на
# котором держится PHASE 4.
#
# ЗАПУСКАЕТСЯ ТОЛЬКО ПОСЛЕ ЗЕЛЁНОГО АГЕНТА, и это не вкусовщина: образ
# собирается из исходников раннера в /opt/linkeon-host-agent, которые кладёт
# туда ИМЕННО установщик агента. Не сошёлся агент — исходники на машине не те,
...
ensure_product_image() {
  ...
  if [[ ! -f "$roll" ]]; then
    red "  ✗ нет $roll — выкатывать образ нечем"
    return 1
  fi

  # Режим наблюдателя (SMOKE_ONLY/PRODUCTS_HOST_CHECK_ONLY) — это сухой прогон
  # скрипта: расхождение находим и докладываем, на машину не пишем ничего.
  local rc=0
  if [[ -n "${PH_CHECK_ONLY:-}" ]]; then
    DRY_RUN=1 SOURCE_SHA="$PH_PROD_SHA" bash "$roll" "$PH_TARGET" || rc=$?
  else
    SOURCE_SHA="$PH_PROD_SHA" bash "$roll" "$PH_TARGET" || rc=$?
  fi

  case "$rc" in
    0) green "  ✓ образ и контейнеры на «${PH_ID}» соответствуют прод-коммиту ${PH_PROD_SHA:0:8}"
       return 0 ;;
    # 3 — «ничего не сломано, но сделано не всё»: чаще всего продукт выполнял
    # правку и его контейнер сознательно не трогали. Фазу это всё равно красит:
    # «машина синхронна» при отставшем контейнере — ровно то враньё, ради
    # которого фаза написана. Но текст обязан отличать пропуск от поломки,
    # иначе оператор пойдёт чинить исправную машину.
    3) red "  ✗ выкат образа выполнен НЕ ПОЛНОСТЬЮ — какие контейнеры пропущены и почему, в выводе выше."
       red "    Чаще всего это живой ход у продукта: гасить его контейнер нельзя, правка"
       red "    оборвалась бы молча. Ничего не сломано, повторить когда освободится:"
       red "      bash $LOCAL_BACK_DIR/scripts/product-image-roll.sh $PH_TARGET"
       return 1 ;;
    *) red "  ✗ выкат образа среды продукта не удался (код $rc) — подробности в выводе выше"
       red "    Раннер внутри контейнеров остался на прежнем коде: правки ассистентом у"
       red "    продуктов этой машины будут работать по-старому."
       return 1 ;;
  esac
}
...
products_host_one() {
  ...
  ensure_nginx_domains || fails=$(( fails + 1 ))
  # Агент (четвёртая часть) идёт ТОЛЬКО за зелёным product-vhost: агент зовёт
  # его с `--domain`, а старая редакция скрипта этого флага не знает — ровно
  # авария 21.09.2026 с `--asleep`. Пара «агент + product-vhost» обязана
  # стоять из одного коммита, поэтому при красном product-vhost агент не
  # ставится (и считается несошедшимся). В режиме проверки на машину не пишет
  # ни одна часть — там агент сверяется как обычно, чтобы отчёт был полным.
  #
  ...
  if (( ! vhost_ok )) && [[ -z "${PH_CHECK_ONLY:-}" ]]; then
    fails=$(( fails + 1 ))
    bold "[машина $PH_ID 4/5] агент хоста на $PH_TARGET"
    red "  ✗ агента не трогаю: product-vhost на машине не обновился, а новый агент зовёт"
    red "    его с флагами, которых старый скрипт не знает. Образ среды — тоже нет:"
    red "    исходники раннера на машину кладёт установщик агента"
  elif ensure_host_agent; then
    ensure_product_image || fails=$(( fails + 1 ))
  else
    fails=$(( fails + 1 ))
    red "  ✗ образ среды продукта не трогаю: агент хоста не сошёлся, а исходники раннера"
    red "    на машину кладёт именно он — собирать было бы не из того кода"
  fi

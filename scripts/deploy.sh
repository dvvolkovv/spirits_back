...
resolve_prod_source() {
  bold "[хост 0/2] источник выката — коммит, выкаченный на my.linkeon.io"
  ...
  if [[ -n "$local_sha" && "$local_sha" != "$prod_sha" ]]; then
    echo "      локальный HEAD ${local_sha:0:8} другой — это норма: на машину продуктов едет"
...
ensure_product_vhost() {
  local src="$PH_SRC_DIR/scripts/product-vhost"
...
ensure_host_agent() {
  ...
  pending=$(rsync -az --checksum --dry-run --itemize-changes \
              ...
              "$src/" "$PRODUCTS_HOST:/opt/linkeon-host-agent/" 2>/dev/null \
            | grep -vE '^\.' || true)
  ...
  bold "  ставлю агента: scripts/product-host-agent-install.sh $PRODUCTS_HOST (код из ${PH_PROD_SHA:0:8})"
  if ! AGENT_SRC="$src" bash "$LOCAL_BACK_DIR/scripts/product-host-agent-install.sh" "$PRODUCTS_HOST"; then
...
run_products_host_phase() {
  ...
  # Источник обеих частей — один, и определяется он ДО них: обе едут из
  # коммита, выкаченного на проде. Не определился — не трогаем НИЧЕГО (обе
  # части считаем несошедшимися), потому что «поставить хоть что-нибудь» здесь
  # означает поставить непонятно что на машину с двумя боевыми продуктами.
  #
  ...
  # Выкладка убирается ЗДЕСЬ, в единственной точке выхода фазы: ни один из
  # ранних return выше неё не проходит — до resolve_prod_source временного
  # каталога ещё нет.
  [[ -n "$PH_SRC_DIR" ]] && rm -rf "$PH_SRC_DIR"

  if (( fails == 0 )); then
    green "  ✓ PHASE 4 GREEN — машина продуктов синхронна с прод-коммитом ${PH_PROD_SHA:0:8}"
    return 0
  fi

...
EOS
  ...
  # Отказ по чужому каталогу — не «уборка не удалась», а «убирать было нечего и
  # не наше». Спутать их нельзя: совет «rm -rf» под этим сообщением указывал бы
  # на каталог, который мы только что сознательно НЕ ТРОНУЛИ.
  if [[ "$out" == *CLEAN_FOREIGN* ]]; then
    warn "убирать нечего: $(sed -n 's/^CLEAN_FOREIGN //p' <<<"$out" | tail -1)"
    warn "  Репетиция его не заводила и не трогала. Разбираться с ним — руками и осознанно."
    return 0
  fi
...
EOS
  ...
  if ! grep -q '^SETUP_OK' <<<"$SETUP"; then
    ...
    # доберёт остальное и скажет, если что-то осталось. Обещать «за собой
    # убрано» прямо здесь нельзя: часть отказов случается ДО того, как мы
    # что-либо завели, а один из них — отказ трогать ЧУЖОЙ каталог, где убирать
    # нечего и не нужно. Итог уборки печатает она сама, строкой ниже.
  ...
  if [[ -z "$STALE" ]]; then
    ...
    while IFS='|' read -r n img; do
      ...
      if [[ -z "$kind" && "$n" != "$REHEARSE" ]]; then
        ...
        # `${NEW_ID:-…}` здесь стояло и печатало ПОЛНЫЙ идентификатор вслед за
        # укороченным: подстановка по умолчанию срабатывает, когда переменная
        # ПУСТА, а нужно было наоборот. Выходило «→ sha256:79fc5cf99f9asha256:79fc…».
        target="${NEW_ID:0:19}"
        [[ -n "$NEW_ID" ]] || target="(идентификатор будет известен после сборки)"
        plan "пересоздал бы $n: ${img:0:19} → $target"

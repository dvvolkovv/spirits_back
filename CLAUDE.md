...
**Действовать самостоятельно, без уточнения**:
- Код, рефакторинг, баг-фиксы, тесты, деплой через `scripts/deploy.sh`.
- Бэклог: `list / get / update / comment` на `/webhook/admin/backlog`; перевод задач в `in_progress` / `done` с автоматической пометкой в комменте.
- Admin auth: брать refresh-token из `5.45.115.22:/var/lib/synthetic/admin-state.json` (sudo cat), обменивать на access. SMS-церемонию запускать только когда refresh протух.
- Внутренние дизайн-решения, у которых есть явная рекомендация — выбирать её, рационал писать в commit message или в коммент к задаче, а не спрашивать.
- Собственное расписание (Cron / wakeup) без согласования каждого тика.

**Уточнять обязательно**:
- Платные внешние интеграции (новые API-аккаунты, повышение лимитов).
- Изменения биллинга / прайсинга для пользователей.
- Публикация наружу с именем клиента (поддержка — если задача уже на Claude, без спроса; новые external-каналы — спрашивать).
- Force-push, удаление данных / таблиц / веток.
- Дизайн-решения 50/50, где у Claude нет явной рекомендации.

**Дефолт при неопределённости**: обратимые действия (локальные правки, тесты, дев-сервер) — делать; необратимые (отправка наружу, money flow, удаление) — спрашивать.
...
## ⚙️ Сборки и тесты — на тестовой ноде, не на маке

**Решение владельца (15.08.2026): мак не тянет.** Локально гонять `npm test`, `npx jest`, `tsc --noEmit`, `pnpm build` — нельзя: прогоны упираются в таймауты и съедают машину. Всё тяжёлое уезжает на `dv@85.192.61.231` (4 ядра, 7 ГБ, ~90 ГБ свободно).

Это про **прогоны**, а не про доставку. Деплой по-прежнему только через `scripts/deploy.sh` — см. секцию ниже.

**Если ты уже на ноде** (`hostname` — `ugliest-salmon`, рабочие клоны в `~/dev/`): с 24.09.2026 разработка идёт прямо там — сессии Claude в tmux на ноде, мак только окно VS Code. Тогда ssh-обёртки ниже не нужны: `npm ci` и `npx jest src/<каталог> --maxWorkers=2` — прямо в своём клоне `~/dev/spirits_back`. Деплой оттуда же — `bash ~/dev/spirits_back/scripts/deploy.sh`, ключи ко всем хостам на ноде есть. Запрет на живой чекаут действует с любой машины: `~/spirits_back` и `~/spirits_front` на ноде не трогать. `~/ci/` остаётся под прогоны, запускаемые с мака.

**Работать только в CI-клонах `~/ci/`, никогда в `~/spirits_back` и `~/spirits_front`.**
`~/spirits_back` на этой ноде — живой чекаут, из которого работает API `test.linkeon.io` на порту 3001. Переключение веток там уронит тестовый стенд. CI-клоны заведены отдельно и на живой стенд не влияют.

```bash
# 1. Локально: закоммитить и запушить ветку (код на ноду — через git, не rsync)
git push -u origin <ветка>

# 2. На ноде: обновить CI-клон и встать на КОНКРЕТНЫЙ sha
ssh dv@85.192.61.231 'cd ~/ci/spirits_back && git fetch -q origin && git checkout -q <sha>'

# 3. Прогоны там же
ssh dv@85.192.61.231 'cd ~/ci/spirits_back && source ~/.nvm/nvm.sh && npm ci && npx jest src/<каталог> --maxWorkers=2'
```

`--maxWorkers=2` — чтобы оставить ноде ресурс на живой тестовый стенд.

### Postgres на ноде для проверок против живой базы

Юнит-тесты подменяют `pg` целиком, поэтому SQL в них **не выполняется вообще**. Проверять запросы надо на настоящей базе — на ноде для этого есть Postgres на 5432 и роль `dv` (LOGIN CREATEDB, заведена 15.08.2026).

Схема: создать одноразовую базу, засеять минимальными копиями нужных таблиц, инстанцировать сервис с реальным `pg.Client` и напечатать результат. Так на этой фиче нашлись два дефекта, невидимых юнит-тестам: сломанное согласование числа («1 сообщений») и `UPDATE`, который на несуществующем `user_id` отрабатывает без ошибки и трогает ноль строк.

Подключаться через unix-сокет: `new Client({ database: '...', host: '/var/run/postgresql' })` — по TCP роль `dv` попросит пароль.

### Ловушки, на которые уже наступали

- **`source ~/.nvm/nvm.sh` обязателен** в каждой ssh-команде: вне nvm на ноде node не найдётся.
- **pnpm на ноде 11.1.3, на маке 11.5.2.** Флаг `--allow-build` там ещё не существует; блокировку postinstall-скриптов `esbuild` снимает `pnpm config set dangerouslyAllowAllBuilds true` (уже в `~/.npmrc` пользователя `dv`, репозиторий не трогает).
- **Бэк — `npm ci` по `package-lock.json`, не pnpm.** Так ставит `deploy.sh`; pnpm здесь только у фронта и лендинга. До `688f72e` `pnpm install` в бэке молча собирал неполный граф по брошенному lock-файлу, и `nest build` падал на `@livekit/protocol`.
- **Не склеивать `cd` в одну команду на два репозитория** — рабочий каталог переносится между звеньями `&&`, и вторая половина молча отработает в первом репо. Так уже терялся push фронта. Использовать `git -C <путь>`.
- **CI-клон ставить на sha, а не на имя ветки.** В общий чекаут на маке параллельная сессия коммитит в ту ветку, что там выставлена: 15.08.2026 чужая работа по почте поддержки уехала в `feat/balance-awareness` и вместе с ней в origin.
- **Временные скрипты класть в корень репо и удалять сразу** — `ts-node` не резолвит модули проекта из `/tmp`.
...
> Без `?nosms=1` каждый тестовый вход Claude пришлёт владельцу SMS (спам + расход). Чисто служебные номера (`70000000000/79030169187/79169403771/790300xxxxx`) глушатся всегда, флаг не нужен.
...
4. **Внешние зависимости не валят деплой**: проверка генерации картинок завязана
   на платный сторонний Google Imagen (тот же `GOOGLE_AI_API_KEY`, что у Veo). Её
   апстрим-падение (5xx/timeout/квота) → `⊘ SKIP` с предупреждением, НЕ fail —
   иначе блип Google откатывал бы несвязанные деплои. Наша инфра (MinIO/Nginx)
   всё равно проверяется public-fetch'ем при успешной генерации, а 4xx (наш
   конфиг/auth) остаётся фатальным.

Вывод: **зелёный `ALL PHASES GREEN` достоверен — ручная перепроверка не нужна.**
Откат срабатывает только на НАШИХ регрессиях (auth/chat/agents/DB/инфра),
красных на всех попытках; транзиент и внешние сервисы его не триггерят.
...
- `GET /webhook/admin/products/:id` — карточка продукта (архивного и тестового тоже): свой домен, последние 50 ходов (тексты обрезаны до 2000 знаков), 20 заданий. Гашение и снятие — прежние `POST /webhook/products/block|unblock {key}`

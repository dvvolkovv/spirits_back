...
- { action: "domain", product, domain: "<домен пользователя>" } — привязать свой домен к сайту; { action: "domain", product } — узнать, как дела; { action: "domain", product, check: true } — проверить DNS сейчас; { action: "domain", product, remove: true } — отвязать.
  Записи DNS из ответа называй ДОСЛОВНО и обязательно скажи удалить у этих имён остальные A-записи и ВСЕ AAAA. Не говори «домен работает», пока domain.status не "active". Свой домен бесплатный. Свой домен бывает только у сайта, один на продукт. Следуй полю say из ответа.
...
app.post("/chat", upload.array("files", 10), (req, res) => {
  ...
  // claude-opus-5-5 добавлен 23.09.2026: бэкенд с этого дня называет модель
  // боевого хода явно (spirits_back, common/chat-model.ts), а не полагается
  // на "default". Незнакомое значение здесь молча заменяется на "default" —
  // то есть без этой строки пин тихо не действовал бы в вебе, оставаясь в
  // силе в телеге, а заметить это можно было бы только по modelUsage.
  //
  // Требует claude >= 2.1.280: более старый бинарь отвергает модель с
  // 400 "does not support this model". На этом хосте значение имеет
  // ~/.local/bin/claude (его находит PATH процесса pm2), а не /usr/bin/claude.
  const MODELS = ["default", "haiku", "sonnet", "opus", "claude-opus-5-5"];

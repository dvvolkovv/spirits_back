...
Всё, что не зависит от спайка, сделано и отревьюировано. Ветка
`feat/meeting-meet` в обоих репозиториях, не смерджена.

| Задача | Состояние |
|---|---|
| 1. Инфраструктура Attendee | **На владельце** |
| 2. Спайк | **На владельце.** Вопрос 5 закрыт по исходникам, остальные шесть открыты |
...
| 9. Аудио-мост | Готово. Хаб одноразовый, порт на задание |
| 10. Проводка в `agent.ts` | Готово. Комната в `session.start()` не передаётся |
...
| 16. Живая проверка и деплой | **Ждёт Attendee: задачи 1-2 на владельце** |

Прогоны на тестовой ноде: бэкенд — 1376 тестов и чистая прод-типизация,
`voice-host` — 97 тестов и чистая сборка, фронт — 410 тестов и сборка.

**Реализация фичи закончена целиком.** Осталось: поднять Attendee, открыть
диапазон портов, прогнать спайк (пять оставшихся вопросов — все про поведение
Attendee, ни один не блокирует код) и провести живую встречу.

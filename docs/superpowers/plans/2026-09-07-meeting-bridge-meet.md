...
**Цепочка работает целиком, подтверждено живой встречей.** Владелец
разговаривал с Романом в Google Meet шесть минут: звук в обе стороны, гейт по
имени, вызов специалиста с озвученным ответом, резюме и списание. Замеры и
полный разбор четырёх прогонов — в `infra/attendee/README.md`.

Путь оказался не тупиковым, но и не бесплатным: четыре прогона вскрыли
одиннадцать дефектов, которых не увидели ни 1408 тестов бэкенда, ни 128
тестов воркера. Каждый разобран в `infra/attendee/README.md` по прогонам.
Один из одиннадцати — **локальная правка самого Attendee**
(`infra/attendee/patches/mic-label-tolerant.py`), без которой ассистента во
встрече не слышно вовсе; её надо применять после каждого его обновления.

В прод пока не тащим: задача стенда была убедиться, что путь работает.

**Открытый дефект, найденный замерами 10.09.2026:** процесс задания растёт
~12 МБ/мин, причём и в полной тишине, то есть это не накопление контекста.
Подозреваемый — `controller.enqueue()` без проверки `desiredSize` в
`attendee-audio.ts` и `mixed-audio-input.ts`; второй файл означает, что дефект
касается обычных звонков и встреч Taler ID тоже. Разбор и порядок работы — в
`infra/attendee/README.md`.

Ветка `feat/meeting-meet` в обоих репозиториях, не смерджена — решение
владельца 09.09.2026: пока работаем в ней.

| Задача | Состояние |
|---|---|
| 1. Инфраструктура Attendee | Стенд — готово (5 контейнеров, sha 31ebd91, nginx-сниппет, порт 8140). Прод — **на владельце**, отдельным хостом |
| 2. Спайк | **Все семь вопросов закрыты** 09.09.2026: 1-4 живой встречей, 5-7 по исходникам. Вопрос 7 закрыт отрицательно: сброса очереди у Attendee нет, перебивание лечится темпом отправки на нашей стороне |
| 3. Разбор ссылки Meet | Готово. Ревью спеки и качества пройдены. Попутно исправлен дефект: битый код своей комнаты скрывал валидную ссылку другой площадки |
| 4. Подпись вебхука | Готово. Канонизация подтверждена золотым вектором из настоящего Python |
| 5. Клиент Attendee | Готово. `removeBot` различает «бота нет» и «состояние неизвестно» |
| 6. Типы дата-канала | Готово |
| 7. Presence | Готово. Имя бота сравнивается нормализованным (пробелы, юникод, регистр) |
| 8. Микшер на любой частоте | Готово. Потолок буфера остался 0.5 с на обеих частотах, проверено |
| 9. Аудио-мост | Готово. Порт на задание; хаб **владеет** сокетом и переживает переподключение Attendee — иначе обрыв обрывал встречу (живая встреча 09.09.2026) |
| 10. Проводка в `agent.ts` | Готово. Комната в `session.start()` не передаётся. Обработчик `meet_*` один и регистрируется сразу после `ctx.connect()`: ниже по файлу он оказывался после ожидания звука и терял весь состав встречи |
| 11. Вебхуки Attendee | Готово |
| 12. Вход `provider='meet'` | Готово. Бот выводится при любом отказе входа |
| 13. Карточка в чате | Готово |
| 14. Фронт | Готово. Плашка «ждём, пока впустят», ключ во всех семи локалях |
| 15. Уборка бота | Готово, с отдельным проходом повторных попыток и частичным индексом |
| 16. Живая проверка и деплой | Живая встреча пройдена на стенде 09.09.2026, три дефекта исправлены. Деплой в прод — **на владельце**: нужен отдельный хост под ботов |

Прогоны на тестовой ноде: бэкенд — 1398 тестов и чистая прод-типизация
(`tsconfig.build.json`), `voice-host` — 115 тестов и чистая сборка, фронт —
410 тестов и сборка.

**Реализация фичи закончена целиком, и путь подтверждён живой встречей.**
Осталось на владельце: отдельный хост под ботов в проде и переход с
Celery-режима на изоляцию по поду, без которой потолок — одна встреча за раз
(см. предупреждение в начале `infra/attendee/README.md`).

Из семи вопросов спайка два закрыты чтением исходников, а не встречей:
канонизация подписи Attendee (вопрос 5) и запуск сессии без LiveKit-комнаты
(вопрос 3, он же был единственным блокером задач 9-10).

**Найдено по ходу и исправлено сверх плана:**

- Расширение `MeetingProvider` до трёх значений **сломало прод-сборку** в
  `chat.service.ts` — прогон jest по каталогу этого не видит, потому что
  ts-jest компилирует файлы по отдельности. Отсюда обязательный шаг проверки
  типов по `tsconfig.build.json` (см. ниже).
- В `join()` при отказе `UPDATE external_bot_id` бот оставался в встрече
  навсегда: id терялся, и реапер, ищущий по непустой колонке, такую строку не
  находил. Плюс `const bot` был объявлен внутри блока и в `catch` недоступен.
- Проход реапера по `external_bot_id` шёл seq scan'ом по всей истории звонков
  каждые 5 минут — подходящего индекса не было, а `voice_calls_active_idx`
  частичный на противоположное условие.
- Тест `customMarkdown.test.ts` во фронте был **красным на `main`** ещё до этой
  работы, с момента появления Taler ID. Незамеченным держался потому, что
  `deploy.sh` гоняет smoke, а не `pnpm test`.

## Дефект плана, найденный ревью 07.09.2026: порт хаба

**Task 10, Step 1 был неверен.** Он ставил `new AttendeeAudioHub(8138)` на
уровне модуля `agent.ts`. Это несовместимо с процесс-моделью
`@livekit/agents@1.7.0`, проверено по установленным исходникам:

| Факт | Где |
|---|---|
| Каждое задание — отдельный процесс через `fork()` | `ipc/job_proc_executor.js:51` |
| Дочерний процесс делает `await import(moduleFile)` **сразу при старте**, в теле IIFE, до получения задания | `ipc/job_proc_lazy_main.js:175-176` |
| После одного задания процесс завершается | `ipc/job_proc_lazy_main.js:261` |
| Слот пула освобождается, когда задание **забирает** прогретый процесс, и пул тут же греет замену | `ipc/proc_pool.js:43` |

Следствие при `numIdleProcesses: 1`: первое же задание **любого** типа забирает
процесс, держащий порт, пул поднимает замену, замена падает с `EADDRINUSE`.
Ломается не только Meet, а способность пула греть процессы вообще — хаб
создавался безусловно. Комментарий «один хаб на процесс воркера» описывал
долгоживущий процесс, обслуживающий много заданий; в этой версии SDK это не так.

### Решение владельца (07.09.2026): порт на задание

Хаб переезжает **внутрь entry-функции** и поднимается только для Meet, на
свободном порту из ограниченного диапазона. Порядок вызовов инвертируется:
порт знает тот, кто им владеет.

```
join() на бэкенде          воркер (задание)              бэкенд (internal)
─────────────────          ────────────────              ─────────────────
INSERT voice_calls
ensureRoom
dispatchAgent ───────────► поднял хаб, узнал порт
                           собрал wsUrl
                           POST …/internal/meet-bot ────► createBot(wsUrl)
                                                          UPDATE external_bot_id
                           ◄──────────────────────────── ok / failed
                           ждёт подключения Attendee
                           Attendee подключился ────────► звук пошёл
```

**Что меняется:**

- `join()` больше **не создаёт бота** — уходят вызов `createBot`, запись
  `external_bot_id` и уборка бота в `catch`. Последняя становится ненужной:
  в момент отказа `join()` бота ещё не существует.
- `join()` больше не может ответить «бот недоступен» сразу. Отказ приезжает
  вебхуком `meet_bot_state` и виден в карточке. Это осознанная потеря.
- Появляется подписанная ручка `POST /webhook/voice-call/internal/meet-bot`
  с телом `{callId, wsUrl}` — рядом с остальными вызовами воркера, под тем же
  HMAC.
- `AttendeeClient.createBot` принимает `wsUrl` параметром вместо чтения env.
- `AttendeeAudioHub` становится **одноразовым**, на задание: занимает первый
  свободный порт из диапазона, умеет отдать публичный URL и закрывается в
  shutdown-колбэке. Метод `close()` наконец начинает вызываться.

**Что выживает без изменений:** `AttendeeAudioInput`, `AttendeeAudioOutput` и
все 8 их тестов, `presence.ts`, `mixer.ts`, контроллер вебхуков, карточка,
фронт, проход реапера.

**Инфраструктура.** Диапазон портов должен быть достижим с хоста Attendee.
Ephemeral-диапазон открывать нельзя — слишком широкая дыра. Берём узкий
именованный диапазон (`ATTENDEE_WS_PORT_MIN`/`MAX`, по умолчанию 8140-8179 —
это же и потолок одновременных встреч) и терминируем TLS на nginx, вынося
порт в путь:
...
публичной сети — для переговоров клиентов это неприемлемо.
...
### Прогон по каталогу НЕ заменяет проверку типов

**Обязательно на каждой задаче, трогающей бэкенд:**

```bash
ssh dv@85.192.61.231 'cd ~/ci/spirits_back && source ~/.nvm/nvm.sh && npx tsc --noEmit -p tsconfig.build.json'
```

Именно `tsconfig.build.json` — это конфиг, который идёт в прод. Задачам в
`voice-host` вместо него `cd voice-host && npm run build`.

Причина конкретная: Task 3 расширила `MeetingProvider` до трёх значений и тем
сломала прод-сборку в `src/chat/chat.service.ts:522`, где результат уезжает в
`buildMeetingCard` с более узким типом параметра. Ни `npx jest src/meeting`, ни
ревью по каталогу этого не увидели — ts-jest компилирует файлы по отдельности и
межпроектную типизацию не проверяет. Поломка вылезла бы на деплое.

`npx tsc --noEmit -p tsconfig.json` (без `build`) для этой цели не годится: он
затягивает `worker/`, у которого свой `package.json`, и выдаёт десяток
несвязанных ошибок про ненайденные модули. Плюс четыре предсуществующие ошибки
в `src/voice-call/*.spec.ts` (неверное число аргументов в конструкторах) — они
в build-конфиг не входят и к этой фиче не относятся.
...
| ~~3~~ | ~~`session.start()` без живой комнаты~~ | **ЗАКРЫТ 07.09.2026 по установленной библиотеке**, спайком проверять не нужно. Ответ: **не передавать `room` вовсе**. Доказательства в `@livekit/agents@1.7.0`: `agent_session.d.ts:434` объявляет `room?: Room` необязательным; RoomIO создаётся только в ветке `else if (room && !this._roomIO)` (`agent_session.js:397`), то есть без комнаты его нет; всё дальнейшее обращение к комнате guarded через `if (room && …)`, а `_updateActivity()` — который создаёт `AgentActivity` — вызывается безусловно; `AgentActivity.start()` в конце безусловно зовёт `_resumeSchedulingTask()` (`agent_activity.js:490`), снимая паузу планировщика, которая инициализируется в `true` (`:173`). Отдельно проверено, что `shouldDiscardInputAudio()` (`:1046-1052`) зависит только от прогрева эхоподавления и неперебиваемой реплики — ни от комнаты, ни от связанного участника, то есть пустая комната вход не глушит |
| 4 | Виден ли бот сам себе и **под каким именно именем** | Приходит ли `participant_events.join_leave` про самого бота, и если да — записать `participant_name` ДОСЛОВНО. `Presence` исключает себя сравнением имён, а не uuid, поэтому обрезанное или изменённое Meet имя (`Роман · ассистент Дмитрия` длинное) сделает бота «участником»: гейт по имени никогда не перейдёт в solo, а комната никогда не опустеет. Если имя приходит не байт в байт — переделать исключение на uuid бота из ответа `createBot` |
...
| 6 | Точный контракт создания бота | **Закрыт по исходникам и живым запросам 09.09.2026.** Приняты `meeting_url`, `bot_name`, `metadata`, `websocket_settings.audio{url,sample_rate}`, `webhooks[{url,triggers}]`, `recording_settings{format: mp4\|mp3\|none}`, `automatic_leave_settings` (набор и дефолты — `bots/automatic_leave_configuration.py`). Ответ — объект бота с `id` и `state`. Валидаторы требуют `wss://` для звука и `https://` для вебхуков |
| 7 | Есть ли сброс очереди звука | **Закрыт по исходникам 09.09.2026: сброса НЕТ.** `on_message_from_websocket_audio` принимает единственный триггер `realtime_audio.bot_output`, остальное — «unknown message» в лог; `RealtimeAudioOutputManager.audio_queue` опустошается только в `cleanup()` при остановке бота. Лечится на нашей стороне темпом отправки (не гнать кадры быстрее реального времени) — отдельной задачей после живой проверки звука, см. комментарий у `clearBuffer` |
...
```ts
    await session.start({
      ...
      // Для Meet комнату НЕ передаём вовсе — разговор идёт вне LiveKit, и
      // привязывать сессию некуда. Это не обходной путь, а поддержанный
      // фреймворком режим: `room` объявлен необязательным, RoomIO создаётся
      // только когда комната передана, а планировщик речи снимается с паузы
      // безусловно в конце AgentActivity.start(). Проверено по исходникам
      // @livekit/agents@1.7.0 07.09.2026, подробности — в вопросе 3 спайка.
      //
      // Наша комната при этом всё равно нужна и подключена через ctx.connect():
      // по ней приходит job и работает дата-канал с присутствием. Просто
      // сессия о ней не знает.
...
**Реапер обязан различать три исхода `removeBot`**, иначе тип возврата
останется декорацией:

| Исход | Что делать |
|---|---|
| `true` | Бот выведен. Обнулить `external_bot_id` |
| `false` | Бота уже не было (404). Тоже обнулить — состояние известно |
| `null` | **Состояние неизвестно** (Attendee недоступен или ответил ошибкой). `external_bot_id` НЕ обнулять и повторить на следующем цикле |

Различие не теоретическое: контейнер Attendee перезапускается, и если считать
`null` успехом, Chrome тихо останется сидеть в переговорах клиента — ровно тот
отказ, ради предотвращения которого `removeBot` и существует.

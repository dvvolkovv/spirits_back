 ...
 *
 * Считается НЕ по технике, а по человеку: звук приходит только после того, как
 * хозяин встречи нажмёт «Впустить», а он может этого и не заметить сразу.
 * Двух минут (первая редакция) хватало лишь когда человек стоял рядом и ждал:
 * 09.09.2026 бот отстучался в комнату ожидания, а мы сдались раньше, чем его
 * впустили, — звонок ушёл в `failed` с причиной «звук так и не подключился»,
 * хотя со встречей всё было в порядке.
 *
 * Шестнадцать минут — чуть БОЛЬШЕ, чем `waiting_room_timeout_seconds` у
 * Attendee (900 с по умолчанию), и это специально: пусть первым сдаётся он,
 * потому что он умеет назвать причину (`waiting_room_timeout_exceeded`
 * приезжает вебхуком и попадает в базу), а наш таймаут причины не знает.
 * Столько же, сколько LOBBY_MS в occupancy.ts, — та же мысль про «человек
 * может опоздать».
 *
 * Ждать бесконечно всё равно нельзя: задание повиснет, фреймворк убьёт его как
 ...
 * поэтому ожидание ничего не стоит, кроме занятого порта.
 */
export const CONNECT_TIMEOUT_MS = 960_000;
  ...
  private adopt(ws: WebSocket, url: string): void {
    ...
    ws.on('close', () => {
      ...
      this.grace = setTimeout(() => {
        ...
        this.onLostCb?.();
        // И сразу отпускаем порт.
        //
        // Ждать больше нечего: бот не вернулся, а порт у нас ОДИН на весь
        // воркер, и пока он занят, следующая встреча получает отказ «порт
        // занят — Meet уже держит одну встречу». Прежде закрытие висело на
        // завершении процесса задания, а процесс живёт ещё долго после конца
        // разговора: 22.09.2026 владелец не смог позвать ассистента трижды
        // подряд, хотя встреча давно закончилась.
        this.close();
      }, this.graceMs);
      this.grace.unref?.();
    });
...
export class AttendeeAudioInput extends voice.AudioInput {
  // Выравнивание громкости не наше дело: мост отдаёт готовый микс, в котором
  // участники уже сведены. Второй параметр — размер тика: 24 кГц против 48 у
  // LiveKit.
  private mixer = new Mixer(false, SAMPLES_PER_TICK);
...
export class AttendeeAudioOutput extends voice.AudioOutput {
   ...
   *
   * ВНИМАНИЕ, подтверждённое ограничение: недосказанное всё ещё лежит в
   * очереди Attendee и договорится поверх нового собеседника.
   *
   * Команды сброса у Attendee НЕТ — прочитано по исходникам 09.09.2026
   * (вопрос 7 спайка закрыт): `bot_controller.on_message_from_websocket_audio`
   * принимает ровно один триггер, `realtime_audio.bot_output`, а всё
   * остальное пишет в лог как «Received unknown message from websocket».
   * Очередь `RealtimeAudioOutputManager.audio_queue` опустошается только в
   * `cleanup()`, то есть при остановке бота.
   *
   * Значит лечится это на НАШЕЙ стороне — темпом отправки: если не гнать
   * кадры быстрее реального времени, в очереди Attendee будет лежать не
   * несколько секунд речи, а несколько сотен миллисекунд. Ровно так работает
   * AudioSource в своих комнатах: `captureFrame` там ждёт места в буфере, и
   * перебивание слышно сразу. Отдельной задачей — после того, как сквозной
   * звук в Meet проверен живой встречей: менять темп вывода одновременно с
   * проверкой звука значит не понять потом, что именно сломалось.
   */
  clearBuffer(): void {

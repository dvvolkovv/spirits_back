...
/**
 * Просьбу уйти исполняет платформа, но модель обязана знать о ней: иначе она
 * начнёт спорить («точно завершать?») или продолжит разговор поверх
 * собственного прощания.
 */
describe('просьба уйти', () => {
  const s = flat(meetingInstructions({
    name: 'Роман', persona: '', preamble: '', specialists: SPECIALISTS,
  }));

  test('сказано прощаться и замолкать', () => {
    assert.match(s, /Хорошо, до свидания/);
    assert.match(s, /Не спорь, не переспрашивай/);
  });
});

...
describe('NameGate', () => {
  ...
  test('режим слушателя не вечен — сам отпускает по истечении срока', () => {
    // Встреча 14.09.2026: на второй минуте прозвучало «Роман, пока слушай»,
    // обратной команды не сказал никто, и ассистент промолчал все 49 минут —
    // 311 решений silent подряд. Участники решили, что он их не слышит.
    const gate = new NameGate('Роман', 30_000, 120_000, 600_000);
    gate.decide('Роман, пока слушай', 1000);
    assert.equal(gate.decide('Роман, что скажешь?', 500_000), 'silent');
    assert.equal(gate.decide('Роман, что скажешь?', 700_000), 'respond');
  });

  test('отпустив режим слушателя, гейт остаётся строгим', () => {
    // Истечение срока возвращает поведение «молчу, пока не позвали», а не
    // «говорю сам»: иначе ассистент через десять минут влезет в разговор,
    // из которого его специально убрали.
    const gate = new NameGate('Роман', 30_000, 120_000, 600_000);
    gate.decide('Роман, пока слушай', 1000);
    assert.equal(gate.decide('а дальше что делать будем?', 700_000), 'silent');
  });

  test('повторная команда слушать продлевает срок', () => {
    const gate = new NameGate('Роман', 30_000, 120_000, 600_000);
    gate.decide('Роман, пока слушай', 1000);
    gate.decide('Роман, пока слушай', 500_000);
    assert.equal(gate.decide('Роман, что скажешь?', 900_000), 'silent');
  });

  test('срок по умолчанию конечен', () => {
    // Значение по умолчанию — то, с которым гейт работает в бою: если оно
    // окажется бесконечным, тесты выше этого не заметят.
    const gate = new NameGate('Роман', 30_000);
    gate.decide('Роман, пока слушай', 0);
    assert.equal(gate.decide('Роман?', 9 * 60_000), 'silent');
    assert.equal(gate.decide('Роман?', 61 * 60_000), 'respond');
  });
...
/**
 * Просьба уйти со встречи.
 *
 * Отдельная команда, а не разновидность «помолчи»: ассистент не замолкает, а
 * выходит, и вернуть его можно только карточкой из чата. Отсюда узкий и
 * повелительный список формулировок — цена ложного срабатывания здесь выше,
 * чем у остальных команд гейта.
 */
/**
 * Кириллица и `\w`.
 *
 * В JavaScript `\w` — это `[A-Za-z0-9_]`, русских букв в нём нет. Команды
 * гейта писались с `\w*` на хвостах слов и работали лишь по случайности
 * (совпадал корень), а «в режиме слушателя» не работало вовсе.
 */
describe('NameGate — русские окончания в командах', () => {
  test('«в режиме слушателя» уводит в режим слушателя', () => {
    const gate = new NameGate('Роман', 30_000);
    assert.equal(gate.decide('Роман, побудь пока в режиме слушателя', 1000), 'ack_listen');
  });

  test('«заканчивай встречу» — просьба уйти', () => {
    const gate = new NameGate('Роман', 30_000);
    assert.equal(gate.decide('Роман, заканчивай встречу', 1000), 'leave');
  });
});

describe('NameGate — просьба уйти', () => {
  const gate = () => new NameGate('Роман', 30_000);

  test('«Роман, завершай» уводит ассистента', () => {
    assert.equal(gate().decide('Роман, завершай', 1000), 'leave');
  });
  ...
  test('синонимы тоже', () => {
    for (const phrase of [
      ...
      'Роман, можешь идти',
      'Роман, ты свободен',
      'Роман, выйди из встречи',
      'Роман, покинь созвон',
      'Роман, заканчивай встречу',
    ]) {
      assert.equal(gate().decide(phrase, 1000), 'leave', phrase);
    }
  });

  test('без имени в комнате с людьми — не команда', () => {
    // Иначе «давайте завершать» в чужом разговоре выгоняло бы ассистента.
    const g = gate();
    g.setSolo(false);
    assert.notEqual(g.decide('давайте завершать, время вышло', 1000), 'leave');
  });

  test('наедине принимается без имени', () => {
    const g = gate();
    g.setSolo(true);
    assert.equal(g.decide('всё, завершай', 1000), 'leave');
  });

  test('«отключи звук» — не просьба уйти', () => {
    // Повелительное «-сь» обязательно: иначе просьба про микрофон уводила бы
    // ассистента со встречи.
    assert.notEqual(gate().decide('Роман, отключи звук у видео', 1000), 'leave');
  });

  test('«выйди» без встречи — не команда', () => {
    assert.notEqual(gate().decide('Роман, выйди из дома пораньше', 1000), 'leave');
  });

  test('просьба уйти сильнее режима слушателя', () => {
    const g = gate();
    g.decide('Роман, пока слушай', 1000);
    assert.equal(g.decide('Роман, завершай', 2000), 'leave');
  });
});

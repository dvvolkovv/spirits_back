...
export default defineAgent({
  entry: async (ctx: JobContext) => {
    ...
    if (isBridged) {
      ctx.room.on(RoomEvent.DataReceived, (payload, _p: any, _k: any, topic?: string) => {
        ...
        if (msg.type === 'meet_chat') {
          ...
          //
          // Сессии может ещё не быть — тогда ответить нечем, но сообщение уже
          // в буфере и достанется модели, когда та о нём спросит.
          if (session && (priv || addressedByName(text, agentName))) {
            pushLine(
    ...
    const tools = {
       ...
       *
       * Единственный канал, где ссылки, названия и имена приходят точно, а не
       * как их расслышала модель: на живой встрече 11.09.2026 в чат прилетело
       * `https://ru.wikipedia.org/wiki/Lighttpd` — попробуй расслышать такое
       * на слух.
       *
       * Инструментом, а не контекстом: класть каждое сообщение в контекст
       * модели оказалось нельзя (см. комментарий у meetingChat), да и незачем
       * — чат нужен только когда о нём спрашивают.
       */
      read_meeting_chat: llm.tool({
        description:
        ...
        parameters: z.object({}),
        execute: async () => {
          if (!meetingChat.length) return { messages: [], note: 'в чат встречи пока никто не писал' };
          return {
            messages: meetingChat.map((m) => ({
              who: m.who,
              text: m.text,
              // Личное сообщение — просьба тебе, а не реплика встречи: об этом
              // модель должна знать, чтобы не пересказывать его вслух всем.
              private: m.private,
      ...
      /**
       * Написать в чат встречи.
       *
       * Зачем вообще: ссылку, номер или адрес голосом не передать — их
       * переспрашивают и всё равно записывают с ошибкой. Чат для этого и
       * существует, и мост умеет в него писать (`send_chat_message`).
       *
       * Всегда ВСЕМ. Личная переписка ассистента с участником — другой
       * продукт со своими ожиданиями (решение владельца 13.09.2026).
       *
       * Ответ честный: `sent: false` означает, что сообщения в чате нет, и
       * модель обязана сказать это вслух. Сбой штатен — бот мог ещё сидеть в
       * комнате ожидания.
       */
      write_meeting_chat: llm.tool({
        description:
          'Написать сообщение в общий чат встречи. Зови, когда просят «скинь в чат», ' +
          'и когда передаёшь ссылку, адрес, номер, код или короткий список — голосом ' +
          'такое не передать. Сообщение видят ВСЕ участники. Без эмодзи.',
        parameters: z.object({
          text: z.string().describe('Текст сообщения. Коротко, без эмодзи.'),
        }),
        execute: async ({ text }) => {
          const body = String(text ?? '').trim();
          if (!body) return { sent: false, reason: 'пустое сообщение' };
          try {
            const r = await backend.meetingChat(meta.callId, body);
            console.log(`[чат] ${r.sent ? 'отправлено' : 'НЕ отправлено'}: ${body.slice(0, 80)}`);
            return r.sent
              ? { sent: true }
              : { sent: false, reason: 'мост не принял сообщение — скажи вслух, что написать не вышло' };
          } catch (e: any) {
            console.error('[чат] отправка не удалась', e?.message);
            return { sent: false, reason: 'связь с чатом встречи потеряна — скажи вслух, что написать не вышло' };
          }
        },
      }),

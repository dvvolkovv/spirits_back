...
export class ProvisioningService implements OnModuleInit, OnModuleDestroy {
  ...
  async create(input: CreateInput): Promise<{ productId: string }> {
    ...
    // МАШИНА ВЫБИРАЕТСЯ ПЕРВОЙ ИЗ ДВУХ ПРОВЕРОК ПО БАЗЕ, И ПОРЯДОК ОСОЗНАННЫЙ.
    //
    // Отказ «мест нет» пользователь починить не может ничем — ни слагом, ни
    // повтором. Отказ «слаг занят» чинится одним словом. Сообщи мы сначала про
    // слаг, человек переименовал бы продукт, нажал снова и только тогда узнал,
    // что заводить его некуда. Обратный порядок ничего не портит: обе проверки
    // стоят ДО выпуска токена и любой записи.
    //

 ...
 * узнавать их сервер обязан по ней. Вид в маркере — именно "domain" (так его
 * кавычит JSON.stringify агента): сервер знает, что спрашивал, и отказ на
 * любой другой вид — не этот случай. Сверяется только НАЧАЛО текста: в
 ...
 */
// Живёт здесь, а не в domains.service.ts: его читает и приём отчёта
// (provisioning.service.ts), которому незачем тянуть сервис ради строки.
export const AGENT_OUTDATED_MARKER = 'неизвестный вид задания: "domain". Агент умеет';

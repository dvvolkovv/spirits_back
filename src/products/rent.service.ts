...
/**
 * ═══ ПОЧЕМУ ПРОБУЖДЕНИЕ ПОДМЕТАЕТСЯ, А НЕ ВЫЗЫВАЕТСЯ ИЗ ЗАЧИСЛЕНИЯ ═══
 *
 * План предлагал позвать `wakeAffordable(userId)` из места зачисления токенов,
 * подразумевая, что оно одно («плюс путь купона»). Мест не два. Баланс в этом
 * проекте растёт ТРИНАДЦАТЬЮ путями, и вот они, проверены чтением кода
 * 17.09.2026:
 *
 *   через SQL-функцию `add_user_tokens`, из TypeScript — девять:
 *     payments.service.processSucceededPayment   (ЮKassa)
 *     priem.service.handleCallback → он же       («Приём», крипта)
 *     payments.service.redeemCoupon              (промокод)
 *     identity.service.issueWelcomeBonus         (приветственные 25 000)
 *     auth.service.debugAddTokens                (правка админом)
 ...
 *     наследие n8n); в этом репозитории её не зовёт никто.
 *
 *   и ПРЯМЫМ `UPDATE ai_profiles_consolidated SET tokens = COALESCE(tokens,0) + $1`,
 *   мимо процедуры и мимо учёта:
 *     referral.service.payoutTokens  (выплата реферального вознаграждения)
 *     referral.service.register      (бонус приглашённому, 20 000)
 *
 * Расставить вызов по девяти TS-местам значит: (1) не закрыть два реферальных
 * и процедуру в базе вовсе; (2) выдать десятому, который напишут через месяц,
 * право забыть — молча, потому что забытое пробуждение выглядит как «продукт
 * почему-то не проснулся», а не как ошибка.
 *
 * Поэтому пробуждение повешено на СОСТОЯНИЕ, а не на событие. Условие «продукт
 * спит, а денег у владельца хватает» не знает, откуда взялись деньги, и потому
 * одинаково верно для всех тринадцати путей, для четырнадцатого, для правки
 * баланса руками через psql и для возврата, пришедшего в обход кода. Событийный
 * крюк закрывает те случаи, которые кто-то вспомнил; подметание закрывает
 * инвариант.
 *
 * ЦЕНА — ЗАДЕРЖКА. Пробуждение наступает не в секунду зачисления, а в пределах
 * оборота ниже. На фоне самого пробуждения (задание → опрос агента хоста →
 * старт контейнера → health-check) это доли общего времени, и владелец в любом
 * случае не увидит сайт мгновенно.
 *
 * ЧТО ЭТИМ ЗАОДНО НЕ СЛОМАНО. Зачисление денег не начинает зависеть от очереди
 * заданий: платёж не ждёт контейнеров и не падает, если пробуждение не удалось.
 * Крюк внутри транзакции зачисления (а тем более триггер в базе) связал бы одно
 * с другим — и отказ на постановке задания откатывал бы оплату.
 */
const WAKE_TICK_MS = 60 * 1000;
...
export class RentService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    ...
    // Отдельный таймер, а не общий со списанием. Списание месячное и ходит раз
    // в сутки; пробуждение обслуживает владельца, который только что заплатил,
    // и суточная задержка была бы для него неотличима от «деньги пропали».
    // Учащать при этом списание нельзя — не потому, что оно сломается
    // (`paid_until <= now()` держит), а потому, что это лишняя правка смысла
    // ради экономии одного setInterval.
    this.waker = setInterval(() => this.safeWakeTick(), WAKE_TICK_MS);
    this.waker.unref?.();
  }
  ...
  private safeWakeTick() {
    this.wakeTick().catch((e: any) =>
      this.logger.error(`оборот пробуждения упал: ${e?.message ?? e}`),
    );
  }
  ...
  /**
   * Оборот пробуждения: обойти владельцев спящих продуктов и разбудить тех, на
   * кого хватает баланса. Возвращает число поставленных заданий.
   *
   * ЭТО И ЕСТЬ ТА САМАЯ «ТОЧКА ЗАЧИСЛЕНИЯ ТОКЕНОВ» — почему она здесь, а не в
   * тринадцати местах, где баланс растёт, см. блок над `WAKE_TICK_MS`.
   *
   * ОТБОР ИДЁТ ПО СПЯЩИМ, А НЕ ПО ПОПОЛНИВШИМ. Список владельцев берётся из
   * реестра продуктов: спящих продуктов на порядки меньше, чем пользователей с
   * движением по балансу, и запрос опирается на существующий частичный индекс
   * `idx_products_paid_until ... WHERE archived_at IS NULL`. Обратный отбор —
   * «кому за минуту зачислили» — потребовал бы читать `token_transactions`,
   * куда два реферальных пути вообще не пишут, то есть вернул бы ровно ту
   * дыру, от которой уходили.
   *
   * ПО ОДНОМУ ВЛАДЕЛЬЦУ И КАЖДЫЙ В СВОЁМ ОПЕРАТОРЕ — по той же причине, что и
   * в `tick`: один битый владелец не должен останавливать пробуждение
   * остальным. Плюс `wakeAffordable` считает бюджет ПЕРСОНАЛЬНО (сколько
   * месяцев владелец может оплатить), и запрос, объединивший всех, обязан был
   * бы повторить эту арифметику вторым экземпляром — а два экземпляра одной
   * арифметики расходятся молча.
   *
   * ПОВТОРНЫЙ ОБОРОТ БЕЗВРЕДЕН. `wakeAffordable` исключает продукты с активным
   * заданием, поэтому продукт, который уже будят (хоть этим оборотом минуту
   * назад, хоть соседним процессом кластера, хоть заданием заведения), второго
   * задания не получает; `ON CONFLICT DO NOTHING` закрывает гонку двух
   * процессов, попавших в одну и ту же микросекунду. Деньги здесь не
   * списываются вовсе — аренду возьмёт ближайший оборот `tick`, когда продукт
   * вернётся в `running`, — поэтому холостой оборот не стоит владельцу ничего.
   *
   * МОЛЧА НЕ ДЕЛАЕТ НИЧЕГО. Строка в лог пишется только когда задания
   * действительно поставлены: оборот ходит раз в минуту, и «разбужено 0» в
   * логе каждую минуту — это шум, в котором тонет всё остальное.
   */
  async wakeTick(): Promise<number> {
    const owners = await this.pg.query(
    ...
    for (const row of owners.rows) {
      try {
        queued += await this.wakeAffordable(row.user_id);
      } catch (e: any) {
        this.logger.error(`пробуждение продуктов ${row.user_id}: ${e?.message ?? e}`);
      }
    }
    if (queued > 0) {
      this.logger.log(`пробуждение: поставлено заданий ${queued}`);
    }
    return queued;
  }
}

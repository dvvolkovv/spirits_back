...
/**
 * Отказ спящему продукту. Отдельный от общего «сейчас недоступен» намеренно, и
 * различие не косметическое.
 *
 * ЧЕМ СОН ОТЛИЧАЕТСЯ ОТ ПРОЧИХ НЕРАБОЧИХ СОСТОЯНИЙ. `stopped` снимает
 * владелец, `provisioning` и `degraded` проходят сами, `failed` чинит повтор
 * заведения — все они либо временные, либо снимаются кнопкой рядом. Сон не
 * пройдёт НИКОГДА сам по себе: продукт спит потому, что на аренду не хватило
 * токенов, и пока баланс не пополнен, ждать нечего. «Попробуйте позже» здесь —
 * прямая неправда, и владелец будет пробовать позже ровно столько раз, сколько
 * у него хватит терпения, ни разу не узнав причины.
 *
 * ПОЧЕМУ 402, А НЕ 409. Ровно тем же кодом ниже отбивается нулевой баланс
 * («Недостаточно токенов»), и причина у обоих отказов одна — деньги. Кабинету
 * нужен один признак «зови пополнение», а не два: 409 он обязан показывать как
 * «агент занят» (это самый частый его источник — замок одного хода), и спящий
 * продукт в этой ветке получил бы чужой текст.
 */
export const SLEEPING_REFUSAL =
...
export class TurnsService implements OnModuleInit, OnModuleDestroy {
   ...
   *
   * Отбор идёт по признаку прогресса (`last_progress_at`), а не по чистой
   * длительности. «Идёт дольше получаса» — догадка о смерти: легитимно
   * длинный ход (крупный рефакторинг плюс сборка плюс тесты) её не переживёт,
   * сборщик снимет с него замок, клиент поставит новый ход поверх того же
   * чекаута, и рядом с живым раннером стартует второй `claude -p` в одном
   * дереве файлов — именно то, ради предотвращения чего существует замок.
   * «Полчаса не подаёт признаков жизни» — свидетельство, а не догадка.
   *
  ...
  async enqueue(input: EnqueueInput): Promise<TurnRow> {
    ...
    // Сон проверяется ЯВНО и ОТДЕЛЬНО, хотя общее условие ниже его и так не
    // пропускает. Расчёт на то, что новый статус «сам собой» не 'running',
    // держится ровно до первой правки этого условия — а цена ошибки здесь
    // молчаливая: ход встаёт в очередь по остановленному контейнеру, забрать
    // его некому (claimNext требует p.status = 'running'), reapStuck хоронит
    // только 'running' — и правка висит в 'queued' навсегда, занимая замок
    // product_turns_one_active. Владелец видит «в очереди» и не получает ни
    // ошибки, ни результата, ни возможности поставить следующий ход.
    //
    // ПЕРЕД проверкой баланса, а не после: у спящего владельца баланса нет
    // почти наверняка (он потому и спит), и порядок наоборот выдал бы ему
    // общее «Недостаточно токенов» вместо объяснения, почему именно продукт
    // не работает и что произойдёт после пополнения.
    if (productStatus === 'sleeping') {
      throw new HttpException(SLEEPING_REFUSAL, HttpStatus.PAYMENT_REQUIRED);
    }

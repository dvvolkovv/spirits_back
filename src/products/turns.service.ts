...
/**
 * Отказ БЛОКИРОВАННОМУ продукту. Третий отдельный от общего «сейчас
 * недоступен», и по той же причине, что у сна: общая ветка отбивает всё, что
 * не 'running', то есть отказ блокированному формально работает и без этой
 * строки — а владелец узнаёт из него ровно ничего.
 *
 * ЧТО ИМЕННО ЛОМАЛОСЬ БЫ МОЛЧА. Продукт остановлен РЕШЕНИЕМ АДМИНИСТРАТОРА, и
 * узнать об этом владельцу больше неоткуда: раздел администратора «Сайты и
 * боты» (GET /webhook/admin/products) ему не виден, уведомлений кусок 4б не
 * делает, единственный след — `block_reason` в карточке и этот текст. Получив «Продукт сейчас недоступен
 * для правок», владелец идёт искать поломку у себя: проверяет домен,
 * перезапускает вкладку, ставит правку ещё раз — и так до тех пор, пока не
 * напишет нам сам.
 *
 * ПРО ПОПОЛНЕНИЕ — ОТДЕЛЬНОЙ ФРАЗОЙ. Соседнее нерабочее состояние (сон)
 * снимается деньгами, и это единственное, что владелец про остановленный
 * продукт уже знает. Не сказать здесь «деньги не помогут» значит собрать
 * оплату за то, что от оплаты не изменится.
 *
 * ПОЧЕМУ 409, А НЕ 402 (код ставится на месте броска, здесь — почему). 402 в
 * кабинете зажигает кнопку пополнения: `setNeedsTopUp(started.status === 402)`
 * в ProductChat.tsx. На блокированном продукте это была бы кнопка, которая
 * берёт деньги и ничего не меняет. 409 кабинет показывает ТЕКСТОМ СЕРВЕРА
 * (`p.message || t('products.chat.busy')`), то есть именно этой строкой.
 */
export const BLOCKED_REFUSAL =
  'Продукт остановлен администратором — правки не принимаются. ' +
  'Пополнение здесь не поможет: напишите нам, если это ошибка.';
...
export class TurnsService implements OnModuleInit, OnModuleDestroy {
  ...
  async enqueue(input: EnqueueInput): Promise<TurnRow> {
    ...
    // ВЫШЕ ОБЩЕЙ ВЕТКИ, и это единственное возможное место. Ниже она была бы
    // мёртвым кодом: `status !== 'running'` отбивает и 'blocked' тоже — значит
    // тест «правка блокированному не ставится» остался бы ЗЕЛЁНЫМ, а владелец
    // получал бы «Продукт сейчас недоступен для правок» на продукт,
    // остановленный решением администратора. Дефект молчаливый по обе стороны:
    // ни ошибки в логе, ни красного теста, ни способа узнать правду.
    //
    // И ВЫШЕ ПРОВЕРКИ БАЛАНСА — по той же причине, по какой там стоит сон:
    ...
    // токенов.
    //
    // 409, а не 402: разбор — в докблоке BLOCKED_REFUSAL.
    if (productStatus === 'blocked') {
      throw new ConflictException(BLOCKED_REFUSAL);
    }

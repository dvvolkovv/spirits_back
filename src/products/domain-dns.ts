 ...
 * сервиса: у долгоживущего канала c-ares ≥1.29 (Node 22.22 и 20.20 — оба на
 * этой версии) урезает таймаут ПОПЫТКИ по истории ответов (5× среднее время
 * ответа, не меньше 250 мс; Node пересчитывает раз в секунду) — на прогретом
 * канале заданные здесь 3000 мс держатся только до третьего ответа, дальше
 * ответ за 1–2.5 с уже даёт ETIMEOUT, хотя сервер жив и просто не мгновенный.

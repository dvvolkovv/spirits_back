...
-- состояний инлайновый, как и все четыре ограничения ниже (форма домена,
-- форма names, словарь причин ошибки, пара «текст — код»): CREATE TABLE
...
CREATE TABLE IF NOT EXISTS product_domains (
  ...
  -- Сертификат Let's Encrypt выпускается на names. Пустым names здесь не
  -- бывает НИКОГДА: пустой список живёт только в задании — сервер собирает
  -- агенту customNames из строки в issuing или active, и у отвязки (removing)
  -- или у продукта без строки вовсе список пуст. Агент понимает НАМЕРЕНИЕ
  -- задания 'domain' именно по этому списку: пустой — снять свои имена и
  -- сертификат, непустой — выпустить и приладить. На этом держится и отвязка
  -- отказавшей заявки при занятом домене (DomainsService.dropOccupiedFailed):
  -- строку удаляют, задание ставят — и агент получает пустой список. Форма
  -- закрыта здесь: только голый корень или корень плюс 'www.' + корень, и
  -- первый элемент обязан совпадать с domain — иначе агент, доверяющий
  -- списку как именам для выпуска, привяжет продукту чужой домен.
  --

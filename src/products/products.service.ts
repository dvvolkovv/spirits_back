 ...
 * её реально использует. С Task 6 плана product_domains станет читать
 * claimJob — при выдаче задания ЛЮБОГО вида любому продукту, не только
 * 'domain', — а с Task 8 её же станет читать список продуктов кабинета.
 * Молча не применившаяся 008 в тот момент — это уже не «нет своего домена», а
 * 42P01 на каждом опросе агента (ни одного заведения, ни одного сна, ни
 * одного пробуждения) и 500 на списке кабинета. deploy.sh jest не гоняет, а
 * прод-smoke продукты не трогает вовсе — необязательная 008 проехала бы
 * зелёными test и smoke и молча уронила бы модуль продуктов на проде уже
 * ПОСЛЕ выката будущих задач, а разбираться пришлось бы по недосчитанным
 * продуктам, а не по строке в логе.
 *
 ...
 * действительно сломанном выкате.
 */

...
export const CHAT_TOOLS = [
  ...
  },
  {
    ...
    description:
      ...
      '• kind — ГЛАВНОЕ различие:\n' +
      '   – "task" (ДЕЛО/НАПОМИНАНИЕ/ЧЕКЛИСТ/ПРИВЫЧКА): то, что «делаешь» и отмечаешь галочкой «сделано» — уход/умывание, приём лекарств/добавок, зарядка, «позвонить», регулярные бытовые дела. МОЖЕТ иметь время и повтор (recurrence), но НЕ занимает блок времени. Кладётся в «Мои дела».\n' +
      '   – "event" (СОБЫТИЕ): настоящая встреча/приём/звонок/выезд с конкретным временем И длительностью — занимает блок в календаре.\n' +
      '   ⚠️ Регулярные дела «утром и вечером каждый день» (уход за кожей, приёмы средств, привычки, чеклисты по расписанию) — это ЗАДАЧИ (kind="task") с recurrence, а НЕ события. Не превращай чеклист в кучу событий.\n' +
      'КАК ЗАДАТЬ ВРЕМЯ (ровно один способ из трёх, для task и event одинаково):\n' +
      '  1) Разово в конкретное время → только datetime.\n' +
      '  2) Регулярный повтор по правилу (каждый день, по будням, раз в N дней/недель) → recurrence + datetime = ПЕРВОЕ вхождение.\n' +
      '  3) Набор конкретных нерегулярных дат → dates (массив ISO-datetime); datetime/recurrence не заполняй.\n' +
      'ВАЖНО: даже если дел несколько или это повтор — вызови инструмент ОДИН РАЗ на весь запрос (через recurrence или dates), НЕ по разу на каждую дату.\n' +
      '• durationMin — длительность в минутах, ТОЛЬКО для event (наличие длительности = событие; для task НЕ указывай).\n• note — короткая заметка (необязательно).',
    input_schema: {
      type: 'object',
      properties: {
        title: { type: 'string' },
        kind: {
          ...
          default: 'event',
          description:
            '"task" — дело/напоминание/чеклист/привычка (уход, приём лекарств, зарядка, «позвонить»): отмечается галочкой, может иметь время и повтор, но НЕ занимает блок времени. "event" — встреча/приём/звонок с временем И длительностью (блок в календаре). Регулярные бытовые/уходовые дела по расписанию = task, НЕ event.',
        },
        datetime: {
          type: 'string',
          description:
            ...
            'Для kind="task" — время дела/напоминания (напр. "2026-08-30T07:30:00"); с recurrence — время первого вхождения серии.',
        },
        recurrence: {
          type: 'object',
          description:
            'Регулярный ПОВТОР события по правилу (каждый будний день / раз в неделю по понедельникам / раз в 2 дня и т.д.). ' +
            'Для ДЕЛ (kind="task", напр. уход утром и вечером каждый день) и для событий. Передавай ВМЕСТЕ с datetime (время первого вхождения). НЕ используй одновременно с dates. ' +
...
export type ToolResult =
  ...
  | {
      ...
      events: { title: string; start: string; end?: string; durationMin?: number; source: string; uid?: string }[];
    }
  | {
      ok: true;
      kind: 'audio';
    ...
    try {
      ...
      if (name === 'read_calendar') {
        ...
        const mapped = events.map((e) => {
          ...
          return { title: e.title, start: toLocalIso(e.at), end: e.end ? toLocalIso(e.end) : undefined, durationMin, source: e.source, uid: e.uid };
        });

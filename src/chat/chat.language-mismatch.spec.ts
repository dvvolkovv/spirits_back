...
describe('код и логи в реплике не делают её иноязычной', () => {
  ...
  // Замерено на этих же строках: CONSOLE_DUMP даёт 74% латиницы (вердикт
  // 'latin' — ровно случай инцидента), CSS_PASTE 58% ('unknown', смешанный).
  // Пороги разводят их по двум разным веткам, и это видно в ожиданиях ниже.

  it('вывод консоли выглядит латиницей — русский ответ не дефект', () => {
    expect(mismatch(RU_LONG, 'ru', CONSOLE_DUMP)).toBe(false);
  });

  it('смешанная реплика с CSS — русский ответ не дефект', () => {
    expect(mismatch(RU_LONG, 'ru', CSS_PASTE)).toBe(false);
  });

  it('на смешанной реплике утечка ВСЁ ЕЩЁ ловится — опора на профиль', () => {
    // 58% латиницы не дотягивают до порога, реплика признана неинформативной,
    // и судим по профилю. Обнаружение здесь сохраняется полностью.
    expect(mismatch(EN_LONG, 'ru', CSS_PASTE)).toBe(true);
  });

  it('на явно латинской реплике утечку пропускаем — осознанная цена', () => {
    // Отличить «человек перешёл на английский» от «человек вставил лог»
    // надёжно нельзя, поэтому молчим. Ложный алерт стоит разбирательства,
    // пропущенный дефект — строчки в статистике.
    expect(mismatch(EN_LONG, 'ru', CONSOLE_DUMP)).toBe(false);
  });
});

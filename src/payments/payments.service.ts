...
export class PaymentsService implements OnModuleInit {
  ...
  async processSucceededPayment(paymentId: string, userId: string) {
    ...
    // ПРОБУЖДЕНИЕ СПЯЩИХ ПРОДУКТОВ ЗДЕСЬ НЕ ЗОВЁТСЯ — И ЭТО НЕ ЗАБЫТО.
    // Продукт, которому не хватило токенов на аренду, засыпает и просыпается
    // сам: `RentService.wakeTick` (src/products/rent.service.ts) раз в минуту
    // подметает спящих, у чьих владельцев баланса снова хватает. Крюк повешен
    // на СОСТОЯНИЕ, а не на событие, потому что баланс в этом проекте растёт
    // тринадцатью путями — включая два реферальных прямым UPDATE и SQL-функцию
    // `redeem_coupon`, работающую внутри Postgres мимо всего TypeScript. Полный
    // перечень и обоснование — в докблоке над `WAKE_TICK_MS` там же.
    //
    // Практический вывод для того, кто добавит четырнадцатый путь: делать
    // ничего не надо. Дописывать вызов пробуждения сюда — тоже не надо: он
    // свяжет зачисление денег с доступностью очереди заданий, и отказ на
    // постановке задания начнёт ронять оплату.

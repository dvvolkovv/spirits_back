...
describe('MeetingController', () => {
  ...
  describe('join: провайдер доезжает до сервиса', () => {
    ...
    it('незнакомый провайдер не проходит за свою комнату молча', async () => {
      ...
      //
      // Пример намеренно взят из будущего: `webex` мост не умеет вовсе.
      // Здесь по очереди стояли `zoom` и `teams` — и каждый раз тест начинал
      // падать в тот день, когда площадку разводили, то есть срабатывал как
      // задумано.
      await ctl.join(user as any, { agentId: 12, code: 'x', provider: 'webex' } as any);

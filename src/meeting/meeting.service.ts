...
export class MeetingService {
  ...
  async join(
    ...
    try {
      ...
      // подключился к комнате и сообщил свою личность отдельной ручкой.
  ...
  /**
   * Воркер подключился к комнате и сообщил свою личность — создаём бота.
   *
   * Идентичность агенту назначает фреймворк, и до подключения её не знает
   * никто, кроме самого воркера. Поэтому вход во встречу (`join`) бота не
   * создаёт: он появляется здесь, уже ПОСЛЕ dispatchAgent. Раньше тем же
   * путём сообщался адрес вебсокета — порядок остался, содержимое поменялось.
   *
   * Ответ синхронный: воркеру нужно знать, есть ли смысл ждать участников
   * встречи, а не тарифицировать Realtime в пустоту.
   */
  async attachBot(callId: string, agentIdentity: string): Promise<{ status: 'ok' | 'failed' }> {
    let call: any;
    try {
    ...
    try {
      ...
      const bot = await this.attendee.createBot({
        ...
        botName,
        callId,
        // Комната задания и есть комната синхронизации: мост зеркалит встречу
        // именно в неё, и воркер уже в ней сидит.
        roomName: String(call.room_name),
        agentIdentity,

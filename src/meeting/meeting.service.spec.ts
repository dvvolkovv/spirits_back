...
describe('MeetingService', () => {
  ...
  describe('встреча Taler ID', () => {
    ...
    it('подписывается в их комнате именем владельца в родительном падеже', async () => {
      // «Роман · ассистент Дмитрий» читается так, будто ассистента зовут
      // Дмитрий: имя приходит из профиля в именительном. Владелец увидел это
      // в списке участников и в чате встречи 14.09.2026.
      withAgent();
      await svc.join('u1', 7, '36fc367a', 'talerid');
      expect(talerIdRooms.join).toHaveBeenCalledWith('36fc367a', 'Андрей · ассистент Дмитрия');
    });

    it('подпись уезжает воркеру готовой', async () => {
      // Тот же текст воркер ставит себе именем в комнате и подписью в чате.
      // Складывать его вторым экземпляром шаблона у себя он не должен: две
      // копии одного правила разъезжаются молча.
      withAgent();
      await svc.join('u1', 7, '36fc367a', 'talerid');
      const meta = livekit.dispatchAgent.mock.calls[0][1] as any;
      expect(meta.assistantDisplayName).toBe('Андрей · ассистент Дмитрия');
    });

    it('своя встреча тоже получает подпись — имя в списке участников оттуда', async () => {
      withAgent();
      await svc.join('u1', 7, 'ABC234');
      const meta = livekit.dispatchAgent.mock.calls[0][1] as any;
      expect(meta.assistantDisplayName).toBe('Андрей · ассистент Дмитрия');

...
export interface CreateBotParams {
  ...
  callId: string;
  /**
   * Токен On-Behalf-Of для Zoom.
   *
   * С 2 марта 2026 Zoom пускает приложение на Meeting SDK во встречу за
   * пределами своего аккаунта только с ним. Токен одноразовый и
   * короткоживущий, поэтому берётся на каждый вход заново и живёт ровно
   * столько, сколько занимает создание бота.
   *
   * Уходит ТОЛЬКО в наш сервис: у Attendee для того же есть свой механизм —
   * внешняя ручка за токенами, — и наш параметр он не поймёт.
   */
  obfToken?: string;
  /**
  ...
  async createBot(p: CreateBotParams): Promise<{ botId: string } | null> {
    ...
    const r = await this.call('/api/v1/bots', {
      ...
      body: JSON.stringify({
        ...
        // Для своего сервиса — токен входа Zoom. Ключ отдельный и необязательный:
        // Attendee такого поля не знает, а лишнее поле в теле он игнорирует.
        ...(p.obfToken ? { zoom_settings: { obf_token: p.obfToken } } : {}),

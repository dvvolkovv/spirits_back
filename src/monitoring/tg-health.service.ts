 ...
 *    отдельный класс отказов, и он тихий. Первый реальный случай (19.08.2026)
 *    оказался не тем, что ожидалось: ответ Claude длиной 4956 символов не
 *    прошёл лимит Telegram, sendMessage вернул 400, исключение ушло наверх до
 *    handleUpdate — тот пишет строку в лог и всё. Юзер не увидел ни ответа, ни
 *    ошибки. Второй сценарий — залипший pg_try_advisory_lock по чату (упал
 *    воркер, не отпустив): каждое следующее сообщение получает «⏳ Минутку»,
 *    пишется в БД и возвращается БЕЗ единого сигнала.
 *
 *    Проверка идёт по answer_expected_at — отметке «бот обязан был ответить»,
 *    которую ставит сам бот в момент решения (см. миграцию
 *    004_tg_bot_answer_watch.sql). Она ловит оба сценария разом, потому что
 *    смотрит на результат (ответа нет), а не на конкретную причину.
 *
  ...
  async checkUnanswered(): Promise<void> {
    ...
    await this.sendUnansweredAlert(
      `<b>🟠 Telegram-бот молчит в ${this.stuckChats.length} чат(ах)</b>\n` +
        ...
        `Причину искать в error-логе прода по chat id: провал доставки (sendMessage) ` +
        `пишет туда полный текст ответа, падение Claude — свою ошибку. ` +
        `Реже — залипший pg_try_advisory_lock по чату: тогда юзер получает «⏳ Минутку» на каждое сообщение.`,
      firstStuck,
    );
  }

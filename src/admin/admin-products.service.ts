...
/**
 * Потолок строк списка — страховка ответа от разрастания, а не пагинация:
 * раздел смотрят глазами, и за пятисотой строкой ищут поиском и фильтрами.
 */
 ...
 * (20 000 продуктов, 1 000 000 ходов, PostgreSQL 16, EXPLAIN ANALYZE):
 * одноуровневый запрос — 0,83 с, из них 0,48 с — компиляция JIT; окно в
 * 365 дней — 2,6 с. Двухуровневый — 0,22 с и 0,35 с: агрегат идёт 500 раз
 * вместо 18 000, JIT без инлайнинга (31 мс). Внешняя сортировка при этом
 * бесплатна — планировщик берёт порядок внутреннего запроса.
 */
...
export class AdminProductsService {
   ...
   * умолчанию: продукты тестовых — это прогоны владельца, и без фильтра они
   * шли бы вперемешку с клиентскими. Порядок — по последней активности

...
describe('устройства в админке', () => {
  ...
  describe('сводка', () => {
     ...
     *
     * Проверка нарочно сужена до запросов-разбивок (они и только они
     * группируют через `GROUP BY 1`). Первая редакция этого теста утверждала
     * `pg.calls.some(...)` — «хоть один запрос считает различных людей», — и
     * оказалась ложным сторожем: тот же счёт есть ещё в трёх запросах сводки,
     * поэтому поломка именно разбивок оставляла тест зелёным. Проверено
     * изломом: подмена COUNT(DISTINCT user_id) на COUNT(*) внутри разбивок
     * не роняла прежнюю редакцию.
     */
    it('в разбивках считает РАЗЛИЧНЫХ людей, а не строки', async () => {
      const pg = fakePg();
      await new AdminService(pg as any).getDeviceStats();

      const buckets = pg.calls.filter((c) => c.sql.includes('GROUP BY 1'));
      expect(buckets).toHaveLength(3); // платформа, ОС, браузер
      for (const b of buckets) {
        expect(b.sql).toContain('COUNT(DISTINCT user_id)');
      }
    });

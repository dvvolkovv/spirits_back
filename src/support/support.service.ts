...
Отвечай только текстом в финальном турне. Никогда не ставь tool-use и финальный ответ одновременно — сначала вызови нужные тулы, потом дай финальный ответ.`;
  ...
  private async performRefund(
    ...
    try {
      ...
      // Поддержке нужен отказ по несуществующему пользователю, а не молчаливый
      // успех, — поэтому существование проверяем сами, и под тем же локом,
      // что возьмёт процедура (после 001_add_user_tokens_lock.sql она читает
      // баланс под FOR UPDATE, так что лок здесь и правда тот же).
      //
      // Прежняя редакция этой заметки гласила «add_user_tokens заводит профиль,
      // если его нет». Это неверно и было неверно всегда: в ветке «строки нет»
      // процедура только объявляет v_new_balance, а UPDATE не находит строку —
      // INSERT там нет. Без проверки ниже возврат ушёл бы в никуда: ответ
      // success, строка в token_transactions есть, баланса нет.

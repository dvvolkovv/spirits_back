...
//
// 17.08.2026: 600 → 750. Курс сиденья в тот же день поднялся с 3 600 до 4 500
// токенов за доллар, то есть один токен стал давать на 20% меньше работы.
// 750/4500 = 600/3600 — выплата токенами сохраняет прежнюю покупательную
// способность, а не дешевеет вместе с балансом. Побочно она стала заметно
// выгоднее покупки: 750 токенов за рубль против ~502 в пакете за 1990 ₽.
const PAYOUT_RATE_TOKENS_PER_RUB = 750;
const PAYOUT_MIN_RUB = 100;

// Вывод ДЕНЬГАМИ (заявка на ручную выплату командой).
//
// 17.08.2026: 1000 → 500. Порог 1000 ₽ на живых данных не брал НИКТО: весь долг
// программы (1 781,20 ₽) разложен по шести лидерам суммами от 14,90 до 771,30 ₽,
// поэтому кнопка «вывести деньгами» была видна, но недостижима. 500 ₽ открывает
// её тем, у кого накопился ощутимый остаток, и оставляет отсечку от переводов
...
export class ReferralService implements OnModuleInit {
  ...
  async getStats(userId: string) {
    ...
    return {
      ...
      // Условия вывода отдаём вместе со статистикой: до этого фронт держал свою
      // копию курса и порогов «совпадает с бэком» — а совпадать они обязаны в
      // двух репозиториях, которые правят и катят по отдельности. Экран считал
      // бы «771,30 ₽ → 462 780 токенов» по старому курсу, зачислили бы 578 475,
      // и это ещё удачный исход: разъехавшийся порог просто запирает кнопку.
      payout: {
        rate_tokens_per_rub: PAYOUT_RATE_TOKENS_PER_RUB,
        min_rub: PAYOUT_MIN_RUB,
        withdraw_min_rub: WITHDRAW_MIN_RUB,

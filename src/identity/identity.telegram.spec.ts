...
describe('список переутверждаемых файлов схемы', () => {
  ...
  it.each(IDENTITY_MIGRATIONS)('%s перечисляет всех провайдеров, если трогает констрейнт', (file) => {
    ...
    //
    // Разбирается только запись `CHECK (provider IN (...))`. Констрейнт,
    // написанный как `provider = ANY (ARRAY[...])`, даст здесь ложно-красный:
    // выражений найдено ноль, тест падает, даже если провайдеры перечислены
    // верно. Это осознанный размен — ложно-красный чинится за минуту, а
    // ложно-зелёный именно в этом тесте уже один раз пропустил урезанный
    // констрейнт. Появится второй синтаксис — расширить регулярку, не гейт.

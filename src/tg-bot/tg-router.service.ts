...
- В ПАПКЕ УЖЕ ЕСТЬ ФАЙЛЫ этого чата — и присланные пользователем, и прошлые результаты:
${workspaceFiles.map(f => `  • ${f}`).join('\n')}
- Пользователь говорит «в файле» без вложения — значит имеет в виду файл из этого списка. ПРОЧИТАЙ его (Read), а не переспрашивай.
- Просят изменить документ — сейчас можно только прочитать исходный файл и выдать исправленную версию ТЕКСТОМ; сохранить новый файл временно нельзя.` : '';

    // В системный промпт добавляем инструкцию про markup для отправки файлов
    // и про доступные инструменты.
    //
    // Безопасность (25.09.2026, решение владельца): из ходов бота ВРЕМЕННО убраны
    // Bash/Write/Edit/Glob/Grep. Эти тулы выполнялись под тем же OS-пользователем,
    // что и API, и владели доступом к ~/spirits_back/.env (JWT_SECRET, доступы к
    // БД); присланный в чат файл или реплика могли довести модель до чтения .env.
    // Пока нет настоящей изоляции (проект в работе), бот только ЧИТАЕТ файлы этого
    // чата (Read в пределах cwd) и ходит в веб (WebSearch/WebFetch). Генерация
    // документов через скрипты недоступна. Вернётся вместе с изоляцией sandbox —
    // тогда сюда возвращаются агентные тулы и обещание авто-прикрепления
    // артефактов, а в generateReply — соответствующий набор tools.
    const sandboxBlock = sandboxDir ? `
- Рабочая папка ЭТОГО чата сохраняется между сообщениями, ты можешь ЧИТАТЬ её файлы (тул Read, только в пределах этой папки).${filesBlock}
- Просят «сделать документ/таблицу/PDF» — генерация файлов сейчас ВРЕМЕННО недоступна. Не обещай прислать файл и не пиши, что «сейчас соберу». Дай результат ТЕКСТОМ в ответе (например, таблицу разметкой), и, если уместно, предупреди, что выгрузка файлом временно отключена.
- Запуск кода, скриптов и установка библиотек — недоступны. Не ссылайся на них и не обещай их использовать.` : '';
...
Пользуйся этим молча: не переспрашивай то, что здесь есть, и не зачитывай этот список вслух.` : '';
    ...
    // Блок про продукты — рядом с остальными возможностями и только там, где
    // инструмент реально выдан: обещать его без инструмента нельзя.
...
${ioInstructions}${productsBlock}${profileBlock}
...
${systemPrompt}`;
    ...
    //
    // ── Безопасность (25.09.2026, решение владельца) ──────────────────────────
    // Bash/Write/Edit/Glob/Grep из ходов бота ВРЕМЕННО убраны: они выполнялись
    // под OS-пользователем API, который владеет ~/spirits_back/.env, и присланный
    // в чат контент мог довести модель до чтения секретов. До появления настоящей
    // изоляции sandbox бот получает:
    //   • `tools` (ДОСТУПНЫЙ набор, CLI-флаг --tools): 'Read,WebSearch,WebFetch'
    //     когда есть рабочая папка чата (cwd = sandboxDir, туда же качаются
    //     вложения — см. tg-bot.service downloadIncomingAttachments/chatWorkspace)
    //     или есть вложения; иначе 'WebSearch,WebFetch' без Read. Read confined
    //     самим CLI пределами cwd — наружу (в т.ч. в .env) он не дотянется.
    //   • `allowedTools` (АВТО-ОДОБРЕНИЕ): 'WebSearch,WebFetch'. Read внутри cwd в
    //     режиме -p и так не спрашивает подтверждения, а Read наружу отклоняется.
    // Возврат агентных тулов — вместе с изоляцией (тогда сюда вернётся прежний
    // набор и обещания генерации файлов в промпте выше).
    //
    // Веб — во всех режимах: без него бот отвечал по обучающим данным и выдавал
    // устаревшее за актуальное молча, без ошибок в логе.
    //
    ...
    // Read нужен там, где есть что читать: рабочая папка чата или вложения (без
    // sandboxDir claude-cli.service сам изолирует вложения в одноразовый cwd).
    const needsRead = !!sandboxDir || !!attachmentPaths?.length;
    const tools = needsRead ? `Read,${WEB_TOOLS}` : WEB_TOOLS;
    // Инструмент продуктов — только автоодобрением: `--tools` задаёт встроенный
    // набор и MCP не режет (проба CLI 2.1.280), а без имени в allowedTools вызов
    // в -p отклоняется.
    const allowedTools = products ? `${WEB_TOOLS},${products.toolName}` : WEB_TOOLS;
    ...
    const { text, costUsd } = await this.claudeCli.textWithCost(userPrompt, {
      ...
      tools,
      allowedTools,
      // Токен владельца — в файл конфига 0600 вне рабочей папки чата: Read этой
      // папки до него не дотянется (claude-cli.service, writeMcpConfig).
      ...(products ? { mcpServers: products.mcpServers } : {}),
    });

...
export interface DeployInput {
  ...
  expectedSha?: string | null;
  /**
   * Освобождает порт продукта от процессов, которых pm2 не ведёт. Зовётся перед
   * КАЖДЫМ перезапуском — и в ходе, и в откате. `report` — строка в фазы хода
   * (с меткой шага), то есть в чат клиента.
   *
   * Без этого сирота на порту переживает всё: pm2 о ней не знает, его рестарт
   * лишь запускает ещё одну копию в занятый порт. Так demo 23.09.2026 откатил
   * правку на диске, а сайт продолжал её отдавать (см. orphans.ts).
   */
...
export async function deploy(input: DeployInput): Promise<{ reverted: boolean }> {
  ...
  const bringUp = async (label: string) => {
    ...
    if (input.restartCmd) {
      // Только когда есть чем поднять: снять держателя порта и ничего не
      // запустить — значит положить сайт своими руками.
      if (input.freePort) {
        try {
          await input.freePort((message) => phase(`${label}: ${message}`));
        } catch (e: any) {
          // Уборка — страховка, а не условие выката: судья здесь — проверка
          // здоровья по sha. Уронить из-за уборки здоровый выкат было бы хуже,
          // чем выкатить поверх сироты и дать sha её поймать.
          phase(`${label}: освободить порт не удалось: ${e?.message ?? e}`);
        }
      }

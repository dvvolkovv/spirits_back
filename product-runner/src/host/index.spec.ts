...
describe('разбор вида задания', () => {
  ...
  it('хостовому шагу уезжают слаг, форма и порт — и больше ничего', async () => {
    // Порт реестра — первый источник: пробуждение обязано вернуть домен на ТОТ
    // ЖЕ порт, с которого сняли. Спрашивать контейнер, когда порт известен,
    // незачем — у него спрашивают только потерянный (см. wakeProduct).

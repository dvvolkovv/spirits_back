...
// Симулятор хоста вынесен в fake-host.ts: его теперь делит sleep.spec.ts, а
// копия разошлась бы с оригиналом молча. Поведение при переносе не менялось —
// собственные свойства симулятора сторожит fake-host.spec.ts.
import { FakeHost, OAUTH, deps } from './fake-host';

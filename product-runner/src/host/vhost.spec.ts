...
describe('vhostArgv', () => {
  ...
  // Повтор дедуплицируется здесь, а не у каждого вызывающего: сон, пробуждение
  // и заведение передают customNames как пришли с сервера.
  it('повтор имени — одна пара --domain, порядок первого появления', () => {
    expect(vhostArgv('product-vhost', 'shop', '--asleep', ['a.ru', 'www.a.ru', 'a.ru', 'www.a.ru'])).toEqual([

...
На проде сейчас **1.10.1** (контейнер поднят с тега `latest` в апреле, поэтому
версия «уехала» сама). В `docker-compose.yml` зафиксирован `v1.10` — сверяйте
перед применением: `sudo docker exec livekit-dozvon livekit-server --version`.
Применить compose с более старым тегом = молча откатить прод на минор назад.
...
отдельный ключ `voicecall*` (заведён 25.08.2026), ключ `dozvon*` не переиспользуется
и оставлен на месте.
